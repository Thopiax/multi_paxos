...
def start config do
  ...
  next config, 0, %{commander: 0, scout: 0}, Map.new, Map.new, Map.new
end # start

defp next config, clock, spawn_count, requests, updates, transactions do
  receive do
  { :spawn_count, type } ->
    new_spawn_count = Map.put(spawn_count, type, spawn_count[type] + 1)
    next(config, clock, new_spawn_count, requests, updates, transactions)
  ...
  { :db_update, db, seqnum, transaction } ->
    ...
    next config, clock, spawn_count, requests, updates, transactions
  ...
  :print ->
    ...
    IO.puts "time = #{clock} commanders spawned = #{inspect spawn_count[:commander]}"
    IO.puts "time = #{clock} scout spawned = #{inspect spawn_count[:scout]}"
    IO.puts ""
    Process.send_after self(), :print, config.print_after
    next config, clock, spawn_count, requests, updates, transactions
